...
(function (definition) {
	...
	/** 
	 * The flowing is a set of webComponents used in our sample templates to illustrate how the API can be leveraged to build full featured UI to control the chart.
	 * 
	 ...
	 * 
	 * This implementation assumes the chart is attached to to quote feed for interactive data loading. 
	 * If you will not be using a quote feed, you will need to adjust these components accordingly.
	 * 
	 *
	...
	CIQ.UI.StudyMenu.prototype.renderMenu=function(){
		...
		for(var i=0;i<alphabetized.length;i++){
			...
			menuItem.append(stx.translateIf(sd.name));
	 ...
	 * 		//
		...
		// just add it to the 'parameters.nameValueStore' like so:
		 ...
		 var UIViewsMenu=new CIQ.UI.ViewsMenu($("cq-views-content"), UIContext, {
		  nameValueStore: new MyNameValueStore(),
		  cb:function(){
		   this.renderMenu();
		  }
		 });

...
<cq-advertisement style="display: block; height: 106px;">
	 ...
	 * to the method to run on that component.
	 *
	...
	Clickable.prototype.setContext=function(context){
		context.advertiseAs(this, "Clickable");
	};
...
<cq-comparison cq-marker>
	...
	FibSettingsDialog.prototype.attachedCallback=function(){
		if(this.attached) return;
		CIQ.UI.ContextTag.attachedCallback.apply(this);
		this.attached = true;
	};

	FibSettingsDialog.prototype.setContext=function(context){
		context.advertiseAs(this, "FibonacciSettingsDialog");
		this.fibTemplate=this.node.find("template");
		this.vectorParameters = context.stx.currentVectorParameters;
	};

	FibSettingsDialog.prototype.open=function(){
	...
	FibSettingsDialog.prototype.setChangeEvent=function(node, section, item){
		...
		function closure(){
			return function(){
				var vectorParameters = self.vectorParameters;
	...
	FibSettingsDialog.prototype.configure=function(){
		var vectorType = this.vectorParameters.vectorType;
		var dialog=$(this);

		// fibonacci type
		var parameters;
		if(this.vectorParameters.fibonacci && vectorType!="fibtimezone") {
			dialog.find(".title").text("Fibonacci Settings");
			var defaultFibs = this.vectorParameters.fibonacci.fibs || {};
			...
			for(var index in defaultFibs) {
				...
				var newParam=CIQ.UI.makeFromTemplate(this.fibTemplate, parameters);
	 ...
	 * @since 4.0.0
	 */
	LanguageDialog.prototype.open=function(){
		this.node.closest("cq-dialog,cq-menu").each(function(){
			this.open();
		});
	};

	LanguageDialog.prototype.setContext=function(context){
		this.initialize();
	};

	LanguageDialog.prototype.initialize=function(){
		...
		function switchToLanguage(langCode){
			return function(){
				...
				CIQ.I18N.setLanguage(stx, langCode);
			};
		}
...
<cq-themes>
   ...
   * @memberof WebComponents.cq-themes
	...
	ShareDialog.prototype.share=function(){
		...
		if(!Promise) console.warn("HTML2Canvas relies upon ES6 Promises. Your browser appears to be missing them, please include a Promise polyfill.");
